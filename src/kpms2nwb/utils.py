...
def find_or_insert_module(nwbfile, module, desc):
    if module in nwbfile.processing:
        return nwbfile.processing[module]
    else:
        return nwbfile.create_processing_module(
            name=module, description=desc)
...
def write_subject_to_nwb(
    ...
    behavior_pm = find_or_insert_module(
        nwbfile, "behavior", "Processed behavioral data")
